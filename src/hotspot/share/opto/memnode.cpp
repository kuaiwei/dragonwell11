...
private:
  ...
  {
    assert(_pointer != NULL, "must always have pointer");
    assert(is_valid == (_base != NULL), "have base exactly if valid");
    assert(is_valid == (_other_offsets != NULL), "have other_offsets exactly if valid");
  }

  static ArrayPointer make_invalid(const Node* pointer) {
    return ArrayPointer(false, pointer, NULL, 0, NULL, 0, NULL);
  }
...
public:
  ...
  static ArrayPointer make(PhaseGVN* phase, const Node* pointer) {
    assert(phase->type(pointer)->isa_aryptr() != NULL, "must be array pointer");
    if (!pointer->is_AddP()) { return ArrayPointer::make_invalid(pointer); }

    const Node* base = pointer->in(AddPNode::Base);
    if (base == NULL) { return ArrayPointer::make_invalid(pointer); }
    ...
    for (int i = 0; i < count; i++) {
      ...
      } else if(int_offset == NULL && parse_int_offset(offset, int_offset, int_offset_shift)) {
        // LShiftL( ConvI2L(int_offset), int_offset_shift)

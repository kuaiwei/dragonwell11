...
private:
  ...
  enum DataOrder { Unknown_order, Forward_order, Reverse_order};
...
public:
  MergePrimitiveStores(PhaseGVN* phase, StoreNode* store) :
    _phase(phase), _store(store), _value_order(Unknown_order)
...
bool MergePrimitiveStores::is_adjacent_input_pair(const Node* n1, const Node* n2, const int memory_size) {
  ...
  // initialize value_order
  if (_value_order == Unknown_order) {
    if (shift_n1 < shift_n2) {
      _value_order = Forward_order;
    } else if (memory_size == 1) {
      _value_order = Reverse_order;   // Only support reverse bytes
    } else {
      return false;
    }
  }

  if ((_value_order == Forward_order && shift_n1 > shift_n2) ||
      (_value_order == Reverse_order && shift_n1 < shift_n2)) {
    // wrong order
    return false;
  }
...
bool MergePrimitiveStores::is_con_RShift(const Node* n, Node const*& base_out, jint& shift_out, PhaseGVN* phase) {
  ...
  if (phase->type(n)->isa_int() != NULL ||
      phase->type(n)->isa_long() != NULL) {
    base_out  = n;
    shift_out = 0;
    return true;
  }
...
Node* MergePrimitiveStores::make_merged_input_value(const Node_List& merge_list) {
  ...
  if (_store->in(MemNode::ValueIn)->Opcode() == Op_ConI) {
    ...
    //           _store                             first
    //
    assert(_value_order != Unknown_order, "sanity");
  ...
  if (_value_order == Reverse_order) {
    if (new_memory_size == 8) {
      merged_input_value = _phase->transform(new ReverseBytesLNode(NULL, merged_input_value));
    } else if (new_memory_size == 4) {
      merged_input_value = _phase->transform(new ReverseBytesINode(NULL, merged_input_value));
    } else {
      return NULL;
    }
  }

...
bool ArenaStatCounter::account(ssize_t delta, int tag) {
  ...
  switch ((MEMTAG)tag) {
    case tag_ra: _ra += delta; break;
    case tag_node: _na += delta; break;
    default: // ignore
      break;
  };
  // Did we reach a peak?
...
public:
  ...
  static unsigned compute_hash(const FullMethodName& n) {
    ...
    return Symbol::compute_hash(n._k) ^
        ...
        */
    return n._k->identity_hash() ^
        n._m->identity_hash() ^
        n._s->identity_hash();
  }

  static bool fmn_equals(const FullMethodName& x1, const FullMethodName& x2) {
    return x1.equals(x2);
  }
...
public:
  ...
  MemStatEntry(FullMethodName method)
    : _method(method), _comptype(compiler_c1),
      _time(0), _num_recomp(0), _thread(NULL),
...
public:
  MemStatTableIterator(size_t min_size, MemStatEntry** flat, int& index, int num_all): _min_size(min_size), _flat(flat), _index(index), _num_all(num_all) {}

  bool do_entry(const FullMethodName& ignored, MemStatEntry* e) {
    if (e->total() >= _min_size) {
      _flat[_index] = e;
      assert(_index < _num_all, "Sanity");
      _index ++;
    }
    return true;
  }
...
class MemStatTable :
    public ResourceHashtable<FullMethodName, MemStatEntry*,
                             FullMethodName::compute_hash, FullMethodName::fmn_equals,
                             7919,
...
public:
  ...
  MemStatEntry** calc_flat_array(int& num, size_t min_size) {
    ...
    iterate_all(do_f);
    */
    MemStatTableIterator closure(min_size, flat, i, num_all);
    iterate(&closure);
    if (min_size == 0) {
      assert(i == num_all, "Sanity");
    } else {
...
static MemStatTable* _the_table = NULL;

void CompilationMemoryStatistic::initialize() {
  assert(_enabled == false && _the_table == NULL, "Only once");
  // _the_table = new (mtCompiler) MemStatTable;
  _the_table = new (ResourceObj::C_HEAP, mtCompiler) MemStatTable();
  _enabled = true;
...
void CompilationMemoryStatistic::on_end_compilation() {
  ...
  {
    MutexLockerEx ml(NMTCompilationCostHistory_lock, Mutex::_no_safepoint_check_flag);
    assert(_the_table != NULL, "not initialized");

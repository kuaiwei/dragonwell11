...
VtableStub* VtableStubs::create_shared_itable_stub() {
  assert(SharedItableStub, "must enable SharedItableStub");
  // Read "A word on VtableStub sizing" in share/code/vtableStubs.hpp for details on stub sizing.
  const int stub_code_length = code_size_limit(false);
  VtableStub* s = new(stub_code_length) VtableStub(false, 0);
  // Can be NULL if there is no free space in the code cache.
  if (s == NULL) {
    return NULL;
  }
  ...
  const Register temp_reg2          = r15;
  const Register icholder_reg       = rscratch2;
  const Register itable_index       = r13;
  Label    L_stub_start;

  const int vtable_start_offset = in_bytes(Klass::vtable_start_offset());
  const int ioffset = itableOffsetEntry::interface_offset_in_bytes();

  const int stub_entries = VtableStubs::shared_itable_entries();

  // we create at le8 itable stub entries
  int i = 0;
  for (; i < stub_entries - 1; i++ ){
    // we calculate offset for lookup_interface_method_stub, any change should be sync
    __ mov(itable_index, (vtable_start_offset + ioffset) - (i * wordSize + itableMethodEntry::method_offset_in_bytes()));
    __ b(L_stub_start);
  }
  __ mov(itable_index, (vtable_start_offset + ioffset) - (i * wordSize + itableMethodEntry::method_offset_in_bytes()));

  __ bind(L_stub_start);
#if (!defined(PRODUCT) && defined(COMPILER2))
  if (CountCompiledCalls) {
    __ lea(r10, ExternalAddress((address) SharedRuntime::nof_megamorphic_calls_addr()));
    __ increment(Address(r10));
  ...
  // Reduce "estimate" such that "padding" does not drop below 8.
  const ptrdiff_t estimate = SharedItableStub ? 180 : 124;
  const ptrdiff_t codesize = __ pc() - start_pc;
  slop_delta  = (int)(estimate - codesize);
  slop_bytes += slop_delta;
  assert(slop_delta >= 0, "shared itable: Code size estimate (%d) for lookup_interface_method too small, required: %d", (int)estimate, (int)codesize);
  ...
  masm->flush();
  // can not bookkeeping for shared stub, the size is larger than regular stub
  // bookkeeping(masm, tty, s, npe_addr, ame_addr, false, itable_index, slop_bytes, 0);
  if (log_is_enabled(Trace, vtablestubs)) {
    log_trace(vtablestubs)("shared itable_stub #[0-%d] at " PTR_FORMAT ": size: %d, estimate: %d, slop area: %d",
                           stub_entries - 1, p2i(s->code_begin()),
                           (int)(masm->pc() - s->code_begin()),
                           stub_code_length,
                           (int)(s->code_end() - masm->pc()));
  }
  guarantee(masm->pc() <= s->code_end(), "overflowed buffer, estimated len: %d, actual len: %d, overrun: %d",
                                         stub_code_length,
                                         (int)(masm->pc() - s->code_begin()),
  ...
  if (PrintAdapterHandlers) {
    tty->print_cr("Decoding shared itable stub [0-%d] @" INTX_FORMAT,
                  stub_entries - 1, p2i(VtableStub::receiver_location()));
  ...
  if (JvmtiExport::should_post_dynamic_code_generated()) {
    JvmtiExport::post_dynamic_code_generated_while_holding_locks("shared itable stub",
...
address VtableStubs::entry_for_shared_itable_stub(int index) {
  assert(SharedItableStub && index < VtableStubs::shared_itable_entries(), "sanity");
  return _shared_itable_stub->entry_point() + index * (2 * NativeInstruction::instruction_size); // mov + br
}
